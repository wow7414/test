...
while True:
    ...
    try:
        ...
        for i in datatable:
            if(i[5] == True):
                data = db(1)
                sql = "SELECT top 1* FROM History WHERE TagName = '%s' AND DateTime >= DATEADD(hh,DATEDIFF(hh,0,getdate()),0) AND DateTime <= DATEADD(hh,1,DATEADD(hh,DATEDIFF(hh,0,getdate()),0))" % i[1]
                dt = data.get_datatable(sql)
                for j in dt:
                    data = db(2)
                    sql = "INSERT INTO Value_Hour values('%s','%s','%s', %s)" % (j[0],j[1],j[25],j[2])
                    data.run_cmd(sql)
    except:
        print("連接不到資料庫")
    print('目前時間:'+datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    s = int(datetime.now().strftime('%M')) * 60
    time.sleep(3660-s)
